...
% PEMS_test
% format short
M=readmatrix("PEMS_test");

%% clear up the data and process the data
[M] = trimbrake(M);
fprintf("Trim finished!");
n_2 = size(M,2);
M = M(:,1:fix(n_2/2));

%%
filename = "traffic.xls";
beta_type = 6;
%    method  part    p
SET=[  1       1     0.2;
       0       1     0.2;
...
for i=1
    tic;
    mainfun(M',SET(i,:),filename,sheets(i),beta_type);
    toc
end

function mainfun(M,SET,filename,sheet,beta_type)
...
Omega = rand(n_1,n_2)<p;
data = M.*Omega;

F=rankmanQR(n_1,n_2,r);
...
settings={  'E(M)',sum(sum(data))/nnz(data);
            ...
            'delta',delta;
            'theta',theta;};
writecell(settings,filename,'Sheet',strcat('set',sheet),'Range','A1');

X0=F.gX0(M);
%X0=X0(1:n_1,:)*X0(n_1+1:end,:)';
% method=0;%apply which descent direction  {1  negetive gradient  0  conjugate direction}
method = SET(1);
outputflag=0;
for way_fsk = [1,0]
    for orth_type = [1,0]
        fprintf("process start!\n");
        ...
        X=X0; 
        if any(isnan(X)),error("Wrong! Line80");end
        t=1;
        time0=clock;
        if(~method)
            gl=F.grad(X);
        ...
        while(t<=max_iter)
            ...
            if(method)
                ...
                beta=F.fbeta(X,gl,g,dl,beta_type);
                d=-g+beta*dl;
            end
            %find stepsize
            if(way_fsk)
...
%       9, output the figure: outplot
    ...
    function Z=gX0(data)
        [U,S,V]=svd(data,"econ"); 
        if any([isnan(U(:));isnan(S(:));isnan(V(:))]);error("Wrong! line264");end
        Z=[U(:,1:r)*sqrt(S(1:r,1:r));V(:,1:r)*(sqrt(S(1:r,1:r)))'];   
    end
    ...
    function [z,ss]=fsk(X,g)
       ...
       if any([isnan(ss)]),error("ss wrong!");end
       z=min(abs(roots(ss)));
    end
