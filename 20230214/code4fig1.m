...
filename="figure1.xls";

% r=floor(11/30*n_1*n_2*p/(n_1+n_2));

SET=[	2000 2000 0 0.05 1 1 1;     %(a)
        2000 2000 0 0.05 0 1 1;     %(b)
        2000 2000 0 0.05 1 1 0;     %(c)
        2000 2000 0 0.05 0 1 0;     %(d)
        ];
for i=1:4
    fprintf("START:%d\n",i);
    maincode(SET(i,:),filename,charset{i},beta_type);
    fprintf("END:%d\n",i);
end
...
for way_fsk = [1,0]   % exact
    for orth_type = [1,0]
        ...
        RMSE(1)=sqrt(sum(sum(H.^2))/nnz(Omega));
